...
- Если специалист Senior/Lead и бриф требует глубокого знания языка или фреймворка — раскрывай внутренности через конкретные задачи. Для Go: goroutines, channels, sync primitives (Mutex, RWMutex, WaitGroup, atomic), context, escape analysis, GC tuning, pprof (CPU/memory/block profiling). Не заменяй это абстрактным "оптимизировал производительность" — показывай ЧЕМ именно.
- Инструменты тестирования: если в оригинале есть конкретные библиотеки (gomock, testify, table-driven tests, pytest, JUnit и т.д.) — сохраняй их явно в навыках и задачах. Не заменяй на абстрактное "Unit/Integration Testing" или "TDD".
...
- Есть ли в задачах конкретные технические детали уровня специалиста (для Senior Go — goroutines/channels/pprof/GC, для Senior Java — JVM tuning/GC logs, и т.д.)? Если нет — добавь в наиболее релевантный проект.
