...
- Мультиагентные паттерны: если бриф требует и в проекте есть LLM/оркестрация — есть ли задача с конкретными паттернами (Orchestrator + sub-agents, fan-out/fan-in, human-in-the-loop)? Формулировки типа "начал исследовать мультиагентные паттерны" — НЕДОПУСТИМЫ. Добавь конкретную задачу.
- NLP/Transformer/embeddings: если бриф требует NLP-экспертизу — есть ли в задачах конкретные модели (BERT, sentence-transformers, seq2seq), типы задач (классификация, NER, семантический поиск), методы (fine-tuning, embeddings)? Если только в навыках — добавь задачу в ML-проект.
- Healthcare safety: если бриф упоминает safety/drift/guardrails для healthcare и в резюме есть медицинский проект — добавлены ли задачи про model drift мониторинг, guardrails, confidence thresholds? Если нет — добавь.
- A/B тесты: если бриф требует и есть production ML сервис — есть ли задача про A/B эксперименты и метрики качества? Если нет — добавь.
- Менторинг: если бриф требует менторинг ML-инженеров и специалист в роли Tech Lead/Senior — есть ли задача про менторинг в соответствующем проекте? Если нет — добавь.
- AI-инструменты разработки (Claude, Codex, Cursor): если бриф требует и есть проект 2023+ — упомянуты ли в стеке и задачах? Если нет — добавь.
- Dataset preparation/model validation: если бриф требует production ML-цикл — есть ли задача про подготовку датасета и валидацию модели? Если нет — добавь в ML-проект.
