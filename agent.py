...
Возьми список всех технологий и требований из брифа. Для каждой пройди два шага:
...
ЗАПРЕЩЁННЫЕ ФОРМУЛИРОВКИ В ЗАДАЧАХ: "начал исследовать", "изучал", "знакомился", "применял принципы X" без конкретики, "начал применять" — это признание что не работал с технологией, а не задача. Задача описывает что специалист конкретно СДЕЛАЛ. Если нужно добавить технологию совместимую с проектом — пиши что сделал, не что "исследовал".
...
Шаг D — СПЕЦИАЛЬНЫЕ ПРОВЕРКИ ДЛЯ ML/AI БРИФОВ (выполняй если бриф содержит ML/AI/LLM требования):
Каждая проверка ниже — обязательная. Если условие выполнено но результат отсутствует — добавь немедленно перед финальным выводом.

1. МУЛЬТИАГЕНТНОСТЬ: бриф требует мультиагентные паттерны → найди проект с LLM/оркестрацией → проверь задачи: есть ли ВСЕ три элемента: Orchestrator + sub-agents, fan-out/fan-in, human-in-the-loop? Если хотя бы один отсутствует — добавь. Пример задачи: "спроектировал мультиагентную систему: Orchestrator-агент декомпозирует запрос и распределяет подзадачи между специализированными sub-agents через fan-out, агрегирует результаты через fan-in; реализовал human-in-the-loop для верификации критических решений перед исполнением".

2. NLP/EMBEDDINGS: бриф требует NLP/embeddings → проверь задачи ВСЕХ проектов: есть ли хоть одна задача с конкретными embedding-моделями или NLP-методами? ВАЖНО: RAG всегда строится на embeddings — если в проекте есть RAG, задача с embedding-моделями ОБЯЗАТЕЛЬНА. Добавь в RAG/ML-проект: "применял sentence-transformers (all-MiniLM, multilingual-e5) для построения семантических векторных представлений документов; настраивал chunking-стратегию и параметры поиска для повышения релевантности RAG-ответов". Если бриф требует seq2seq или классические NLP — добавь отдельную задачу с конкретикой (TF-IDF, токенизация, NER, классификация).

3. A/B ТЕСТЫ: бриф требует A/B тесты → найди production ML-проект → есть ли задача про A/B эксперименты? Если нет — добавь: "проводил A/B эксперименты для сравнения версий моделей и prompt-стратегий, отслеживал метрики качества (precision/recall/F1, BLEU, ROUGE) в production; принимал решения о выкатке на основе статистической значимости".

4. МЕНТОРИНГ: бриф требует менторинг ML-инженеров → найди проект где специалист Tech Lead или Senior → есть ли задача про менторинг? Если нет — добавь в Tech Lead проект: "проводил менторинг Middle/Junior ML-инженеров: ревью ML-кода, разбор архитектурных решений, постановка задач через метрики и критерии приёмки, выстраивание культуры разработки через артефакты а не интуицию".

5. AI-ИНСТРУМЕНТЫ: бриф требует Claude/Codex/Cursor/SDD → найди проект 2023+ → упомянуты в задачах? Если нет — добавь: "использовал Claude и Cursor в SDD-цикле (Specification-Driven Development): написание спецификаций → генерация кода → ревью → тесты; ускорил разработку типовых компонентов".

6. HEALTHCARE SAFETY: бриф требует safety/guardrails для healthcare → проверь: есть ли в резюме проект в медицинском домене который содержал ML/AI компонент? ТОЛЬКО если такой проект есть — добавь задачу про guardrails и model drift. Если медицинский проект был чисто бэкенд (БД, asyncio, API) без ML — НЕ добавляй safety/drift, это будет выдуманный опыт.

7. DATASET/VALIDATION: бриф требует production ML-цикл → найди ML-проект → есть ли задача про подготовку данных и валидацию модели? Если нет — добавь: "организовал pipeline подготовки датасета: разметка, балансировка классов, аугментация; настроил стратифицированный train/val/test сплит и кросс-валидацию для надёжной оценки модели".
...
- Примеры совместимости:
  ...
  Android-стек: Java совместима с любым Android-проектом на Kotlin — Android SDK построен на Java, в Kotlin-проектах всегда присутствует Java interop (Java-библиотеки, legacy-код, Java SDK API). Если Java есть в навыках специалиста и бриф требует Java — ОБЯЗАТЕЛЬНО добавь задачу в реальный проект: "поддерживал совместимость Kotlin-кода с Java-библиотеками через @JvmStatic/@JvmField аннотации", "интегрировал Java SDK компоненты в Kotlin-проект через interop", "рефакторил legacy Java-модули в Kotlin с сохранением публичного API". Нельзя оставить Java только в навыках без упоминания в проектах если бриф это требует. Unit-тесты (JUnit, Mockito/MockK) и UI-тесты (Espresso, UIAutomator) совместимы с любым Android-проектом — если бриф требует тесты и специалист хоть где-то упоминает тестирование (даже в обучении) — добавь конкретную задачу в коммерческий проект: "покрывал ViewModel и UseCase unit-тестами на JUnit 4/5 + MockK, изолируя зависимости через моки", "писал UI-тесты на Espresso для критических пользовательских сценариев". RxJava 2 совместима с любым Android-проектом — если в навыках и бриф требует, добавь задачу: "использовал RxJava 2 для обработки асинхронных потоков (Observable, Single, Completable), применял операторы flatMap/switchMap/debounce для reactive UI".
  ML/AI-стек: Мультиагентные паттерны (Orchestrator + sub-agents, fan-out/fan-in, human-in-the-loop) совместимы с любым проектом где есть LLM, LangChain или AI-оркестрация — если бриф требует мультиагентность, ОБЯЗАТЕЛЬНО добавь задачу в проект с LLM: "спроектировал мультиагентную систему с Orchestrator-агентом, распределяющим задачи между специализированными sub-agents через fan-out паттерн, с агрегацией результатов через fan-in", "реализовал human-in-the-loop для верификации критических решений агентов перед выполнением"; НЕЛЬЗЯ оставить мультиагентность только в навыках. Tool-calling совместим с любым проектом где есть LLM-интеграция — добавь задачу: "разработал набор инструментов (tools) для LLM с чёткими схемами ввода/вывода, реализовал function calling для автоматического вызова бизнес-логики по решению модели". Transformer-архитектуры и NLP (BERT, RoBERTa, sentence-transformers, embeddings, seq2seq) совместимы с любым проектом где есть ML, NLP или RAG — если в навыках есть NLP/Transformer и бриф требует — ОБЯЗАТЕЛЬНО добавь задачи в ML-проект: "применял sentence-transformers для построения семантических embeddings и поиска релевантных документов в RAG-пайплайне", "реализовал пайплайн предобработки текста (токенизация, лемматизация, фильтрация) и построил TF-IDF и dense embeddings для сравнения подходов", "дообучал (fine-tuning) BERT-модель на доменных данных для задачи классификации или извлечения сущностей (NER)"; нельзя оставить Transformer/NLP только в навыках без задач в проекте. A/B тесты совместимы с любым production ML-сервисом — если бриф требует, добавь: "проводил A/B эксперименты для сравнения моделей, отслеживал метрики качества (precision/recall/F1/BLEU) в production через статистические тесты". Dataset preparation и model validation совместимы с любым ML-проектом — если бриф требует production ML-цикл: "организовал pipeline подготовки датасета (разметка, балансировка, аугментация), настроил cross-validation и holdout-оценку модели". Healthcare AI safety и guardrails совместимы с любым проектом в медицинском домене (MIS, EHR, медицинские устройства, клинические данные) — если бриф требует safety и проект медицинский: "разработал систему guardrails для валидации ML-ответов в клиническом контексте: confidence thresholds, fallback на rule-based логику при низкой уверенности, фильтрация галлюцинаций", "настроил мониторинг model drift с алертами при деградации качества предсказаний". AI-инструменты разработки (Claude, Codex, Cursor) с SDD совместимы с любым проектом 2023+ — если бриф требует, добавь в стек и задачу: "использовал Claude/Cursor в SDD-цикле для ускорения разработки: спецификация → генерация кода → ревью → тесты". Менторинг ML-инженеров совместим с любой ролью Tech Lead/Senior в ML-проекте — если бриф требует менторинг: "проводил менторинг Middle/Junior ML-инженеров: ревью ML-кода, разбор архитектурных решений, выстраивание культуры разработки через метрики и артефакты а не интуицию".
